...
- рекламный текст;
- карточку оффера;
- кнопки: `Изменить время`, `Отклонить` (а после согласования — `Изменить время`, `Отказаться`).
...
- до момента публикации блогер может менять слот (`Изменить время`) даже после автосогласования (`scheduled`);
...
- в момент слота без взятия в работу оффер уходит в архив;
...
- ошибка автопоста -> `auto_publish_error`;
- отказ блогера -> `declined_by_blogger`;
- истёк слот без взятия в работу -> `archived_not_published`;
- отмена рекламодателем -> `cancelled_by_advertiser`.
...
- Если у блогера несколько каналов, `/pause` сначала просит выбрать канал.

## 6. Выбор времени
- В Telegram-чате (inline-кнопки): пагинация по датам + слоты выбранной даты + `Назад`.
- В WebApp: выбор даты и списка доступных слотов этой даты (`date + select`).
...
- Фильтры в WebApp:
  ...
  - `Запланированные`:
    ...
    - для `auto_with_precheck`: офферы `pending_precheck` и `scheduled`.
- Для `pending_precheck` доступны: `Изменить время`, `Отклонить`.
...
- В WebApp доступно `Опубликовать сейчас`: оффер переводится в `scheduled` на ближайшие ~2 минуты (без ожидания ближайшего слота), если текущий момент внутри окна кампании и публикация технически возможна.
