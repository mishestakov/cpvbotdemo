...
Блогер получает карточку оффера и 2 действия:
...
- Цвета кнопок: `Выбрать другое время` (синяя), `Отклонить` (красная).
...
UX в этом режиме:
- На каждый оффер создаётся отдельный topic в личке бота.
- Все сообщения по офферу идут в этот topic (карточка, обновления, итог).
- Цвета кнопок: `Подтвердить` (зелёная), `Выбрать другое время` (синяя), `Отклонить` (красная).
...
- При переходе в финальный статус бот отправляет в topic итог (статус + сумма начисления), после чего topic закрывается.
