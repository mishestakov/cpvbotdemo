...
- Работать в режиме "ленивая реализация по умолчанию":
  - сначала самый короткий рабочий вариант;
  - заглушка допустима, если пользователь явно не просил production-ready;
  - не делать "красиво заранее", если это не влияет на текущий сценарий.
- Бюджет сложности по умолчанию для одной доработки:
  - не более 1-2 файлов и ~30-40 строк изменений;
  - если нужно больше, сначала остановиться и согласовать с пользователем.
- Не делать рефакторинг "заодно":
  - трогать только код, который нужен для текущего запроса;
  - улучшения вокруг задачи предлагать отдельно, но не внедрять без подтверждения.
- Предпочитать уменьшение кода:
  - если можно решить удалением/упрощением, выбирать это;
  - если можно не добавлять новый слой абстракции — не добавлять.
